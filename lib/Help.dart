...
import 'package:flutter_std/model/ModelSysData.dart' as prefix0;
...
const METHOD_LOGIN = "login/autologin";
const METHOD_DEPTANDUSERTREELIST = "department/deptAndUserTreeList";
const METHOD_MOBILEMENULIST = "menu/mobileMenuList";
const METHOD_SYSDATA = "sysdata/dataList?dataCode=";
const METHOD_GETCUSTOMERS = "customer/getCustomers";
const METHOD_DETAILBYBUSINESSKEYS = "flowinstance/detailbybusinesskeys";
const METHOD_PROJECTINFO = "projectinfo/get/";
const METHOD_PROJ_MAIN_PROJECT =
    "flowmodelnode/getAllNodesByRefTable/PROJ_MAIN_PROJECT";
const METHOD_FLOWMODELNODE = "flowmodelnode/candidates?";
const METHOD_COMPLETE = "flowtask/complete";
const METHOD_BACK = "flowtask/back";
const METHOD_STATEMAP = "flowinstance/statemap";
const METHOD_ADD = "projectinfo/add";
const METHOD_PROJECTINFOUPDATE = "projectinfo/update";
const METHOD_PROJECTINFOLIST = "projectinfo/list";
const METHOD_TODOPROJECTFORM = "todo/projectform";
const METHOD_UPLOAD = "http://192.168.0.6:8002/api/file/upload"; //上传文件
const METHOD_DELETE = "attachment/deleteBatch/"; //删除文件
...
class Help {
  ...
  static ModelSysData mModelSysData_PROJECT_TYPE;

  static ModelSysData mModelSysData_CUSTOMER_REGION;

  static ModelSysData mModelSysData_GENGER;

  static ModelSysData mModelSysData_TASK_TYPE;

  static ModelSysData mModelSysData_URGENCY_DEEP;
  static ModelSysData mModelSysData_TASK_SHAPE;

  static ModelSysData mModelSysData_TASK_SOURCE;

  static ModelCustomer mModelSysData_METHOD_GETCUSTOMERS;
...
//  }
  ...
  static String getRightDate(num date) {
    if (date == 0) {
      return "";
    }
  ...
  static String getNameFormId(String ids, List data) {
    if (ids == null || ids == '') return '';
    try {
      for (int i = 0; i < data.length; i++) {
        if (data[i] is BaseEmployeeListBean) {
          if (data[i].EmpID == ids) {
            return data[i].EmpName;
          }
        } else if (data[i] is prefix0.ListListBean) {
          if (data[i].itemId == ids) {
  ...
  static String getNamesFormIds(String ids, List data) {
    if (ids == null || ids == '') return '';
    try {
      String names = '';
      ...
      mids.forEach((f) {
        for (int i = 0; i < data.length; i++) {
          if (data[i] is BaseEmployeeListBean) {
            if (data[i].EmpID == f) {
              names += data[i].EmpName + ',';
            }
          } else if (data[i] is prefix0.ListListBean) {
            if (data[i].itemId == f) {
              names += data[i].itemName + ",";
            }
          }
        }
      });
